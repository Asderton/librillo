...
async function eliminar_biblioteca(username_cliente, id_biblioteca) {
    const username_credor = await get_creador_biblioteca(id_biblioteca);
    if (username_credor === undefined || username_credor !== username_cliente){
        return {status: 401, error: "No eres dueño de esta biblioteca"};
    }
...
async function eliminar_libro_biblioteca(username_cliente, id_biblioteca, isbn_code) {
    const username_credor = await get_creador_biblioteca(id_biblioteca);
    if (username_credor === undefined || username_credor !== username_cliente){
        return {status: 401, error: "No eres dueño de esta biblioteca"};
    }
...
async function modificar_biblioteca(username_cliente, id_biblioteca, nombre_biblioteca, icono = null) {
    const username_credor = await get_creador_biblioteca(id_biblioteca);
    if (username_credor === undefined || username_credor !== username_cliente){
        return {status: 401, error: "No eres dueño de esta biblioteca"};
    }

    const biblioteca_actualizada = await db_client.query('UPDATE bibliotecas SET nombre_biblioteca = $1, icono = $2 WHERE id_biblioteca = $3 RETURNING nombre_biblioteca',[nombre_biblioteca, icono, id_biblioteca]);
    if (biblioteca_actualizada.rowCount === 0){
        return undefined;
    }
    return biblioteca_actualizada.rows[0].nombre_biblioteca;
}
